...
export default function User() {
  ...
  useEffect(() => {
    ...
    // Cleanup function เพื่อหยุดการฟังเมื่อ component ถูก unmount
    return () => unsubscribe();
  }, [router]);

  const handleSignOut = () => {
    auth.signOut(); // ใช้ Firebase signOut
    router.push("/auth/login");
  };

  if (loading) {
    return <div>Loading...</div>; // แสดงข้อความ Loading ขณะรอการตรวจสอบสถานะ
  }
