...
export default function Login() {
  ...
  const [loading, setLoading] = useState(false); // เก็บสถานะการโหลด
  const router = useRouter();

  // ฟังก์ชันจัดการการเปลี่ยนแปลงใน input fields
  const handleChange = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
    setError(""); // ล้างข้อความผิดพลาดเมื่อมีการเปลี่ยนแปลง
  };
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true); // ตั้งค่าเป็นสถานะกำลังโหลด

    try {
      // พยายามล็อกอินด้วย Firebase
      ...
      if (user) {
        // หากล็อกอินสำเร็จ บันทึกข้อมูลผู้ใช้ใน localStorage
        localStorage.setItem(
          "user",
          JSON.stringify({
            username: user.displayName || "Anonymous", // ใช้ "Anonymous" ถ้าไม่มีชื่อ
            email: user.email,
          })
        );
        router.push("/parking_space"); // ไปยังหน้าที่ต้องการหลังล็อกอิน
      }
    } catch (err) {
      // แสดงข้อความผิดพลาดจาก Firebase
      const errorMessages = {
